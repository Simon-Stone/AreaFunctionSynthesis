...
TriangularGlottis::TriangularGlottis()
{
  ...
  Parameter cp[NUM_CONTROL_PARAMS] =
  {
    ...
    { "Subglottal pressure", "pressure", "dPa", 1.0, "dPa", 0.0, 20000.0, 10000.0, 0.0 },
...
{
  const int TDS_SAMPLING_RATE = 44100;
  int i;
  ...
  supraglottalPressureFilter.createChebyshev(25.0/(double)TDS_SAMPLING_RATE, false, 4);
...
//  F0+= 0.25*(F0/100.0)*(sin(2.0*M_PI*3.0*12.7*time_s) + sin(2.0*M_PI*3.0*7.1*time_s) + 
  //                      sin(2.0*M_PI*3.0*4.7*time_s));
