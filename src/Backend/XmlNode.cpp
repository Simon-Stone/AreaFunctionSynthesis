...
static XmlNode *parseElement(string &input, int &pos, XmlNode *parent, vector<XmlError> *errors);
static void parseStartTag(string &input, int startPos, int endPos, XmlNode *element, vector<XmlError> *errors);
static string getNextToken(string &input, int &pos, int endPos, vector<XmlError> *errors);
static bool isValidName(string st);
static bool isWhiteSpaceChar(char ch);
static string condenseWhiteSpace(string input);
static void decodeErrorPositions(string &input, vector<XmlError> &errors);
...
XmlNode *xmlParseString(string &input, string tag, vector<XmlError> *errors)
{
  int pos = 0;
  XmlNode *node = NULL;
...
XmlNode *xmlParseFile(string fileName, string tag, vector<XmlError> *errors)
{
...
/*
  string input =
    ...
    "    <param index=\"1\" name=\"F0\" /> \n"
    "      This is some more text inbetween...      \n"
    " </glottis>\n";
*/
  ...
  if (rootNode != NULL)
  {
    printf(rootNode->toXmlString().c_str());
  }
  else
  {
...
int XmlNode::numChildElements(string name)
{
  int count = 0;
  int i;
...
XmlNode *XmlNode::getChildElement(string name, int index)
{
  int counter = 0;
  int i;
  int k = -1;
...
int XmlNode::getAttributeInt(string name)
{
  int i;
  int k = -1;
...
double XmlNode::getAttributeDouble(string name)
{
  int i;
  int k = -1;
...
string XmlNode::getAttributeString(string name)
{
  int i;
  int k = -1;
