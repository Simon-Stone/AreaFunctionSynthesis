...
const double Tube::MIN_AREA_CM2 = 0.1e-2;  // 0.1 mm^2
...
void Tube::setPharynxMouthGeometry(const double *length_cm, const double *area_cm2,
  const Articulator *articulator, const double *laterality, const double incisorPos_cm)
{
  int i;
  Section *ts = NULL;
  double pos_cm = 0.0;
...
void Tube::interpolate(const Tube *leftTube, const Tube *rightTube, const double ratio)
{
  ...
  double laterality[NUM_PHARYNX_MOUTH_SECTIONS];
  ...
  for (i=0; i < NUM_PHARYNX_MOUTH_SECTIONS; i++)
  {
    ...
    laterality[i] = ratio1*leftTube->pharynxMouthSection[i].laterality + ratio*rightTube->pharynxMouthSection[i].laterality;
  }

  // Interpolate the teeth position.

  double incisorPos_cm = ratio1*leftTube->teethPosition_cm + ratio*rightTube->teethPosition_cm;

  // Interploate the articulators of the tube sections.

  // ...


  this->setPharynxMouthGeometry(length_cm, area_cm2, articulator, laterality, incisorPos_cm);
