...
const double TlModel::MIN_AREA_CM2  = 0.1e-2; // = 0.1 mm^2
...
{
  const double TL_CUTOFF_FREQ = 10100.0;
...
void TlModel::prepareCalculations()
{
  ...
  Complex fossaInputImpedance;
  Complex inputImpedance;
...
Complex TlModel::getRadiationCharacteristic(double omega)
{
  const double MICROPHONE_DISTANCE_CM = 25.0;    // 25 cm
  Complex H;
  
  H = Complex(0.0, (AMBIENT_DENSITY_CGS*omega) / (4.0*M_PI*MICROPHONE_DISTANCE_CM));
...
Complex TlModel::getRadiationImpedance(double omega, double radiationArea_cm2)
{
  // Init. the radiation impedance with 0 (no radiation)
  Complex radiationImpedance(0.0, 0.0);
  ...
  if (options.radiation == PISTONINWALL_RADIATION)
  {
    ...
    radiationImpedance = analogImpedance * Complex(0.5*ka*ka, (8.0*ka) / (3.0*M_PI));
  }
  else
  ...
  if (options.radiation == PISTONINSPHERE_RADIATION)
  {
    ...
    radiationImpedance = Complex(R0, omega*L0);
  }
  ...
  {
    Complex Z1 = (128.0*AMBIENT_DENSITY_CGS*SOUND_VELOCITY_CGS) / (9.0*M_PI*M_PI*radiationArea_cm2);
    Complex Z2 = Complex(0.0, (omega*8.0*AMBIENT_DENSITY_CGS) / (3.0*M_PI*sqrt(M_PI*radiationArea_cm2)));
...
void TlModel::getLumpedSectionImpedances(double omega, Tube::Section *ts, Complex &Za, Complex &Zb)
{
  if (ts == NULL)
  {
    Za = 0.0;
  ...
  Za = Complex(0, omega*Ls);
  if (options.boundaryLayer)   { Za+= Rs; }
  if (options.hagenResistance) { Za+= Rf; }

  // Parallel impedance.

  Zb = Complex(0, omega*Cp);
  if (options.heatConduction) { Zb+= Gp; }
  if (options.softWalls)
  { 
    Zb+= 1.0 / (Complex(ts->wallResistance_cgs, omega*ts->wallMass_cgs - ts->wallStiffness_cgs/omega) / (length*circ));
  }
...
Matrix2x2 TlModel::getSectionMatrix(double omega, int section)
{
  ...
  if ((section >= Tube::FIRST_SINUS_SECTION) && (section <= Tube::LAST_SINUS_SECTION))
  {
    ...
    Complex Za = Complex(0, omega*Ls);
    if (options.boundaryLayer) { Za+= Rs; }

    Complex Zb = 1.0 / Complex(Gp, omega*Cp);   // 1.0 / Sum of the conductivities
  ...
  // ****************************************************************
  {
    ...
    Complex z(0, omega*Ls);
    if (options.boundaryLayer)   { z+= Rs; }
    if (options.hagenResistance) { z+= Rf; }

    // The entire conductivity in parallel (per length-unit)
    Complex y(0, omega*Cp);
    if (options.heatConduction) { y+= Gp; }
    if (options.softWalls)
    { 
      y += 1.0 / (Complex(ts->wallResistance_cgs, omega*ts->wallMass_cgs - ts->wallStiffness_cgs / omega) / circ);
    }

    Complex Z0 = std::sqrt(z/y);
    Complex gamma = std::sqrt(z*y);

    // The matrix elements.

    M.A = M.D = std::cosh(gamma*length);
    Complex SINH = std::sinh(gamma*length);
    M.B = SINH * Z0;
    M.C = SINH / Z0;
  }
...
Complex TlModel::getJunctionImpedance(double omega, double A1_cm2, double A2_cm2)
{
  Complex Z;
  ...
  double H = 1 - b/a;
  Z = Complex(0, omega*8.0*AMBIENT_DENSITY_CGS*H / (3.0*M_PI*M_PI*b));
...
Complex TlModel::getInputImpedance(int freqIndex, int section)
{
  Matrix2x2 M;
  ...
  {
    Complex noseInputImpedance  = getInputImpedance(freqIndex, Tube::FIRST_NOSE_SECTION);
    Complex mouthInputImpedance = getInputImpedance(freqIndex, Tube::FIRST_MOUTH_SECTION);
...
Complex TlModel::getOutputImpedance(int freqIndex, int section)
{
  Matrix2x2 M, K;
  Complex lungImpedance = lungTerminationImpedance[freqIndex];
...
Complex TlModel::getPressureSourceTF(int freqIndex, int section)
{
  Matrix2x2 M, K;
  Complex result;
  Complex factor;
  Complex inputImpedance;
  Complex outputImpedance;
  Complex Za, Zb;
...
Complex TlModel::getFlowSourceTF(int freqIndex, int section)
{
  Matrix2x2 M, K;
  Complex result;
  Complex factor;
